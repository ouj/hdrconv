...
static void WriteImageTGA(const string &name, float *pixels,
        ...
        int totalXRes, int totalYRes,
        int xOffset, int yOffset);
static Color *ReadImageTGA(const string &name, int *w, int *h);
static bool WriteImagePFM(const string &filename, const float *rgb, int xres, int yres);
static Color *ReadImagePFM(const string &filename, int *xres, int *yres);

// ImageIO Function Definitions
Color *ReadImage(const string &name, int *width, int *height) {
    if (name.size() >= 5) {
 ...
 */
static Color *ReadImageTGA(const string &name, int *width, int *height)
{
    int                 x, y, pixbytes;
    tga_header_t        header;
...
static bool WriteImagePFM(const string &filename, const float *rgb,
 ...
 fail:
    fprintf(stderr, "Error writing PFM file \"%s\"", filename.c_str());
    fclose(fp);
    return false;
}
